...
class WakeTimeTableViewCell: UITableViewCell {
    ...
    private func deselectOtherButton() {
        ...
        for indexPath in indexPaths! {
            if indexPath.row != tappedCellIndexPath.row {
                let cell = tableView.cellForRow(at: IndexPath(row: indexPath.row, section: indexPath.section)) as! WakeTimeTableViewCell
...
protocol TimeRadioTableViewCellDelegate: AnyObject {
